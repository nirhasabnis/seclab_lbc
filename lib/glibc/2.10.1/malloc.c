...
Void_t*
orig_realloc(Void_t* oldmem, size_t bytes)
{
  mstate ar_ptr;
...
Void_t*
orig_memalign(size_t alignment, size_t bytes)
{
  mstate ar_ptr;
  Void_t *p;
...
/*	Note that we expose the glibc_calloc function and use an alias because
 *	this needed by the mark_guardzone function in libzcheck.a library.
 * */
Void_t*
glibc_calloc(size_t n, size_t elem_size)
{
  mstate av;
  mchunkptr oldtop, p;
...
/*	added by amisra.
 *	This has been done so that orig_calloc is used in glibc. and glibc_calloc
 *	is used externally in the guardzone_markup function.
 * */
extern __typeof(glibc_calloc) orig_calloc 
	__attribute((alias("glibc_calloc"), visibility("hidden")));
...
mutex_t guard_map_acc_lock[MT_ARRAY_SIZE];
...
init_guard_map_acc_lock(void)
{
	...
	for (count=0; count < MT_ARRAY_SIZE; ++count)
		mutex_init (& guard_map_acc_lock[count]);
}
...
#define alloc_guard_map_page(higher_index)	do {								\
	...
	lock_index = (higher_index) % MT_ARRAY_SIZE;							\
																			\
	(void) mutex_lock (& guard_map_acc_lock[lock_index]);						\
	if (guard_map[higher_index] == 0) 										\
		if ((guard_map[higher_index] = 										\
			orig_calloc (GZ_MAP_SIZE, sizeof(unsigned))) == 0)				\
			exit (1);														\
	(void) mutex_unlock (& guard_map_acc_lock[lock_index]);						\
																			\
} while(0)
...
/* 
 * Modified for the same comment as before
 */
/*#define	aligned_post_processing(var1, var2, var3)										\
{																				\
	char *rear_gz;																\
																				\
	rear_gz = (var1) + (var2); 													\
...
/*
 * This is variant of malloc that takes element size
 * as its second argument. We infer the value of
 * element_size by looking at the type to which malloc results 
 * are cast.
 */

Void_t*  public_mALLOc2(size_t, size_t);

Void_t*
public_mALLOc2 (size_t size, size_t element_size)
{
	void *ptr;
	size_t rev_size = round_up(size, chunk_alignment);
	size_t checked_elemsize = check_gz_size_limits(element_size);
	size_t zone_size = round_up(checked_elemsize, chunk_alignment);
...
Void_t*
public_cALLOc (size_t nmemb, size_t size)
{
	void *ptr;
	size_t rev_size = round_up(size*nmemb, chunk_alignment);

	/*	In case of calloc, we have the size of the object to be used	*/
	type_size = size;
	size_t zone_size = calc_gz_size(size * nmemb);
	/*	Calculating new number of elements so that the guardzones can be
	 *	accomodated in their place.
	 *	Note that we will have to take into account the space 
	 *	(rev_size - size) that is required for aligning guardzone on a
	 *	GRANULARITY boundary.
	 */
	size_t add_size = (2 * zone_size + (rev_size - size));
	size_t new_num = nmemb + add_size / size + ((add_size % size) ? 1 : 0);

	if((ptr = orig_calloc (new_num, size)) == 0) {
#ifdef	ZDEBUG
		fprintf(stderr, "calloc: error in libc call.\n");
#endif
		return 0;
	}
...
/*
 * Variant of calloc that takes element size
 * 
 * NOTE: For calloc, we could have considered 'size' parameter
 * itself as an element size, but it is unclear if programmers
 * use this field in true sense. E.g., it is quite possible that
 * instead of using calloc (2 * sizeof (int)), they may actually
 * use calloc (8, 1). To avoid this issue, and to be consistent
 * across all memory allocation functions, we create a variant of
 * calloc also.
 */

Void_t*  public_cALLOc2(size_t, size_t, size_t);

Void_t*
public_cALLOc2 (size_t nmemb, size_t size, size_t element_size)
{
	void *ptr;
	size_t rev_size = round_up(size*nmemb, chunk_alignment);
	size_t checked_elemsize = check_gz_size_limits(element_size);
	...
	if((ptr = orig_calloc (new_num, size)) == 0) {
#ifdef	ZDEBUG
		fprintf(stderr, "calloc2: error in libc call.\n");
#endif
		return 0;
	}
...
/*
 * Variant for realloc -- similar to malloc -- takes element size
 * as input to determine guard zone size. Element size is computed by
 * looking at the type to which realloc results are cast.
 */

Void_t*  public_rEALLOc2(void*, size_t, size_t);

Void_t*
public_rEALLOc2 (void *ptr, size_t size, size_t element_size)
{
	void  *realloc_buffer;
	...
	if (get_guardzone(ptr, &gz_addr)) {
#ifdef	ZDEBUG
		fprintf(stderr, "realloc2 : guardzone error\n");
		return ptr;
#endif
		abort();
	}
	 ...
	 *	guardzones.
	 */
	if((realloc_buffer = public_mALLOc2(size, element_size)) == 0)
		return ptr;
...
public_fREe (void *ptr)
{
	...
	if (addr_gz.gz_front) {
		...
		orig_free(addr_gz.gz_front);
	}
	else 
		orig_free(addr_gz.buffer);
}	
...
public_mEMALIGn(size_t boundary, size_t size)
{
	...
	if((ptr = orig_memalign (boundary, new_size)) == 0) 
		return 0;
...
Void_t*  public_mEMALIGn2(size_t, size_t, size_t);

Void_t *
public_mEMALIGn2(size_t boundary, size_t size, size_t element_size)
{
	void *ptr;
	size_t rev_size = round_up(size, chunk_alignment);
	size_t checked_elemsize = check_gz_size_limits(element_size);
...
/*
 * Variant of valloc that takes element size
 * as input.
 */

Void_t*  public_vALLOc2(size_t, size_t);

Void_t*
public_vALLOc2(size_t size, size_t element_size) 
{
	void *ptr;
	size_t rev_size = round_up(size, chunk_alignment);
	size_t checked_elemsize = check_gz_size_limits(element_size);
...
public_pVALLOc(size_t size)
{
	...
	if((ptr = orig_pvalloc(new_size)) == 0) 
		return 0;
...
Void_t*  public_pVALLOc2(size_t, size_t);

Void_t*
public_pVALLOc2(size_t size, size_t element_size)
{
	void *ptr;
	size_t rev_size = round_up(size, chunk_alignment);
	size_t checked_elemsize = check_gz_size_limits(element_size);
	...
	guardzones guardzone;
	if (get_guardzone(addr, &guardzone))
		fprintf(stderr, "pvalloc2 : %p\nerror in allocating guardzones.\n", addr);
#endif
	return ptr;
}

size_t
public_mUSABLe(void *ptr)
{
	guardzones addr_gz;

	/*	We had an error during retrieval of guardzone data. How are we supposed
	 *	to respond here??	*/
	if (get_guardzone(ptr, &addr_gz)) {

		/*	In this case, we COULD PRETEND that no guardzones exist and see how
		 *	that goes	*/
		return mUSABLe(ptr);
	}

	/*	Else return the value from the guardzone meta-data.	*/
	return addr_gz.buffer_size;
}
