...
#endif
{
	Void_t* orig_realloc(Void_t*, size_t);
  __malloc_hook = NULL;
  __realloc_hook = NULL;
  ptmalloc_init();
  return orig_realloc(ptr, sz);
}
...
#endif
{
	Void_t* orig_memalign(size_t, size_t);
  __memalign_hook = NULL;
  ptmalloc_init();
  return orig_memalign(alignment, sz);
}
